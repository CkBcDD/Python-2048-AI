...
def run_gui(agent: Optional[Callable[[List[List[int]], List[int]], Optional[int]]] = None, size: int = 4):
    pygame.init()
    ...
    def random_valid_action(state: List[List[int]], legal_actions: List[int]) -> Optional[int]:
        # 简单策略：返回第一个合法动作（上、右、下、左优先顺序）
        for a in ACTIONS:
    ...
    if agent is None:
        agent = random_valid_action  # 没提供 agent 时，A 开关使用内置策略
